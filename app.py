...
# 3. FUNCIÓN GENERADORA DE PDF (CON CLÁUSULAS Y FECHA AUTO)
def generar_pdf_io(cliente, items_finales, total, tipo, firma_cli_data, firma_prov_data, fecha_p, notas):
    pdf = FPDF()
    pdf.add_page()
    
    # Encabezado (Espacio corregido para no encimar logo)
...
    pdf.ln(18)
    ...
    pdf.set_font('Arial', '', 10)
    pdf.multi_cell(0, 5, "EL PRESTADOR se compromete a instalar un sistema de camaras (CCTV). Los equipos se detallan a continuacion:")
    pdf.ln(2)
...
    pdf.ln(5)
    ...
    pdf.set_font('Arial', '', 10); pdf.multi_cell(0, 5, f"El costo total es de ${total:,.2f} MXN. La instalacion se programa para el {fecha_p.strftime('%d/%m/%Y')}.")
    
    pdf.ln(2); pdf.set_font('Arial', 'B', 10); pdf.cell(0, 5, "CLAUSULA TERCERA: GARANTIAS", ln=True)
    pdf.set_font('Arial', '', 9); pdf.multi_cell(0, 4, "- Equipos: 8 meses de garantia contra defectos de fabrica.\n- Mano de Obra: 4 meses de garantia en instalacion y configuracion.")

    pdf.ln(2); pdf.set_font('Arial', 'B', 10); pdf.cell(0, 5, "CLAUSULA CUARTA: ANULACION DE GARANTIA", ln=True)
    pdf.set_font('Arial', '', 8); pdf.multi_cell(0, 4, "La garantia se anula por: Manipulacion de terceros, daños electricos (voltaje), daño fisico, mal uso, vandalismo o problemas de internet ajenos al equipo.")
...
        p_cli = guardar_temp(firma_cli_data)
        p_prov = guardar_temp(firma_prov_data)
        
        pdf.image(p_cli, 30, y_f, 50, 25)
        pdf.image(p_prov, 130, y_f, 50, 25)
