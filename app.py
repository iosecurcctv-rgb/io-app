...
def generar_pdf_io(cli, items, total, tipo, sub_m, periodo, tec, n_cam, can, f_c_img, f_p_img, f_ini, dom, notas):
    ...
    if tipo == "Servicio IO Prevent":
        ...
        pdf.multi_cell(0, 5, f"De una parte, IO SECURITY, en adelante 'EL PRESTADOR'. Y de otra parte, {cli}, en adelante 'EL CLIENTE'. Ambas partes acuerdan celebrar el presente CONTRATO DE SOPORTE TÉCNICO REMOTO para un sistema de {n_cam} camaras tecnologia {tec} con grabador de {can} canales.")
        ...
        v_m = 6 if periodo == "Semestral" else 12; f_t = f_ini + timedelta(days=30*v_m)
        pdf.multi_cell(0, 4.5, f"La duracion de este contrato es de {v_m} meses ({periodo.lower()}) a partir de la firma. \nFecha de Inicio: {f_ini.strftime('%d/%m/%Y')} \nFecha de Termino: {f_t.strftime('%d/%m/%Y')}"); pdf.ln(1)
        
        pdf.set_font('Arial', 'B', 9); pdf.cell(0, 5, "TERCERA: ALCANCE TECNICO LIMITADO", ln=True); pdf.set_font('Arial', '', 9)
        pdf.multi_cell(0, 4.5, "Verificacion de Grabacion y Salud del Disco Duro (Diagnostico Remoto mensual) y Soporte y Configuracion de Aplicacion Movil (Hasta 4 celulares/tablets)."); pdf.ln(1)
        
        pdf.set_font('Arial', 'B', 9); pdf.cell(0, 5, "CUARTA: EXCLUSIONES (LO QUE NO INCLUYE)", ln=True); pdf.set_font('Arial', '', 9)
        pdf.multi_cell(0, 4.5, "Reubicacion de equipos, Reparacion de cableado, conectores o fallas electricas y Refacciones. Visita fisica de emergencia costo preferencial de $200.00 MXN."); pdf.ln(1)
        
        pdf.set_font('Arial', 'B', 9); pdf.cell(0, 5, "QUINTA: HORARIO DE ATENCION", ln=True); pdf.set_font('Arial', '', 9)
        pdf.multi_cell(0, 4.5, "Soporte remoto: Lunes a Viernes 9:00 AM a 6:00 PM y Sabados 9:00 AM a 2:00 PM."); pdf.ln(1)
        
        pdf.set_font('Arial', 'B', 9); pdf.cell(0, 5, "SEXTA: CONFIDENCIALIDAD Y ACCESO", ln=True); pdf.set_font('Arial', '', 9)
        pdf.multi_cell(0, 4.5, "EL PRESTADOR se compromete a mantener estricta confidencialidad sobre las credenciales de acceso. El acceso se limitara a tareas de diagnostico."); pdf.ln(1)
        
        pdf.set_font('Arial', 'B', 9); pdf.cell(0, 5, "SEPTIMA: HONORARIOS", ln=True); pdf.set_font('Arial', 'B', 10)
        pdf.cell(0, 7, f"El costo total por el servicio {periodo.lower()} es de ${total:,.2f} MXN, pagaderos en una sola exhibicion.", ln=True)
        ...
        pdf.multi_cell(0, 5, f"EL PRESTADOR se compromete a instalar y poner en funcionamiento un sistema de camaras de seguridad (CCTV) en el domicilio ubicado en {dom}. Detalle:"); pdf.ln(2)
        ...
        pdf.ln(2); pdf.set_font('Arial', 'B', 10); pdf.cell(140, 8, ' TOTAL NETO A PAGAR', 1, 0, 'L'); pdf.cell(50, 8, f"$ {total:,.2f} ", 1, 1, 'R'); pdf.ln(3)
        
        pdf.set_font('Arial', 'B', 10); pdf.cell(0, 5, "CLAUSULA SEGUNDA: VIGENCIA Y COSTO", ln=True); pdf.set_font('Arial', '', 10)
        pdf.multi_cell(0, 5, f"El costo total es de ${total:,.2f} MXN. Inicio: {f_ini.strftime('%d/%m/%Y')}. Finalizacion estimada mismo dia."); pdf.ln(2)
...
with st.container():
    ...
    if tipo == "Servicio IO Prevent":
        ...
        with st.expander("🌐 CONFIGURACION IO PREVENT", expanded=True):
            ...
            can_io = e2.selectbox("Grabador Canales:", ["4", "8", "16", "32"])
            total_final = m_total
