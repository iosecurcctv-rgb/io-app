...
# 3. GENERADOR DE PDF (CORRECCIONES APLICADAS)
def generar_pdf_io(cli, items, total, tipo, sub_m, periodo, tec, n_cam, can, f_c_img, f_p_img, f_ini, dom, notas):
    ...
    # NOMBRE DE LA EMPRESA HASTA ARRIBA EN TODOS LOS CONTRATOS
    ...
    if tipo == "Servicio IO Prevent":
        ...
        pdf.set_font('Arial', '', 9); pdf.cell(0, 5, f'LUGAR: Pachuca de Soto/Hidalgo.', ln=True, align='R'); pdf.ln(5)
        ...
        # TABLA DE EQUIPO CUBIERTO (IO PREVENT)
        ...
        pdf.multi_cell(0, 4.5, "La prestacion de servicios de asistencia tecnica y monitoreo de salud del sistema de videovigilancia de manera exclusivamente remota."); pdf.ln(1)
        
        pdf.set_font('Arial', 'B', 9); pdf.cell(0, 5, "SEGUNDA: VIGENCIA", ln=True); pdf.set_font('Arial', '', 9)
        f_t = f_ini + timedelta(days=365) # VIGENCIA DE UN AÑO SIEMPRE
        ...
        # TÍTULO DINÁMICO PARA MANTENIMIENTO
        titulo_final = f"Contrato de Prestacion de Servicios de {tipo if tipo != 'Mantenimiento' else 'Mantenimiento ' + sub_m}"
        pdf.set_font('Arial', 'B', 12); pdf.cell(0, 10, titulo_final, ln=True, align='C'); pdf.ln(5)
        ...
        pdf.multi_cell(0, 5, f"Instalacion/Mantenimiento en el domicilio ubicado en {dom}. Detalle:"); pdf.ln(2)
        ...
        clausulas = [
            ...
            ("CLAUSULA QUINTA: ANULACION", "Se anula por: Voltaje, manipulacion ajena, daño fisico o desastres."),
...
sub_m = st.sidebar.selectbox("Subtipo Mantenimiento:", ["Preventivo", "Correctivo"]) if tipo == "Mantenimiento" else ""
...
with st.container():
    ...
    if tipo == "Servicio IO Prevent":
        ...
        with st.expander("🌐 CONFIGURACION IO PREVENT", expanded=True):
            ...
            total_final = m_total # COSTO MANUAL HABILITADO
        ...
        with st.expander("🛠️ CONFIGURACIÓN MANTENIMIENTO", expanded=True):
            ...
            m_prorr = st.multiselect("💰 CARGAR GANANCIA EN:", opc_mnt)
