...
# 1. IDENTIDAD Y ESTILO (NO SE MODIFICA NADA VISUAL)
...
# 3. GENERADOR DE PDF (CON TEXTOS LITERALES Y CORRECCIÓN DE ERROR)
def generar_pdf_io(cli, items, total, tipo, sub_m, periodo, tec, n_cam, can, f_c_img, f_p_img, f_ini, dom, notas):
    ...
    pdf.ln(32) # MARGEN DE SEGURIDAD PARA QUE NO SE ENCIME EL LOGO
    ...
    if tipo == "Servicio IO Prevent":
        ...
        pdf.multi_cell(0, 5, f"De una parte, IO SECURITY, en adelante 'EL PRESTADOR'. Y de otra parte, {cli}, en adelante 'EL CLIENTE'.")
        pdf.ln(2); pdf.multi_cell(0, 5, "Ambas partes acuerdan celebrar el presente CONTRATO DE SOPORTE TÉCNICO REMOTO, regido por las siguientes:")
        
        pdf.ln(2); pdf.set_font('Arial', 'B', 10); pdf.cell(0, 5, "CLAUSULAS", ln=True); pdf.set_font('Arial', 'B', 9)
        pdf.cell(0, 5, "PRIMERA: OBJETO DEL SERVICIO", ln=True); pdf.set_font('Arial', '', 9)
        pdf.multi_cell(0, 4.5, "La prestacion de servicios de asistencia tecnica y monitoreo de salud del sistema de videovigilancia de manera exclusivamente remota.")
        
        pdf.ln(2); pdf.set_font('Arial', 'B', 9); pdf.cell(0, 5, "SEGUNDA: VIGENCIA", ln=True); pdf.set_font('Arial', '', 9)
        f_t = f_ini + timedelta(days=365)
        pdf.multi_cell(0, 4.5, f"La duracion es de un (1) año. Inicio: {f_ini.strftime('%d/%m/%Y')} - Termino: {f_t.strftime('%d/%m/%Y')}.")

        pdf.ln(2); pdf.set_font('Arial', 'B', 9); pdf.cell(0, 5, "SEPTIMA: HONORARIOS", ln=True); pdf.set_font('Arial', 'B', 10)
        pdf.cell(0, 7, f"Costo total por servicio {periodo.lower()}: ${total:,.2f} MXN.", ln=True)
    else:
        # CONTRATO INSTALACIÓN LITERAL (TEXTO DE IVÁN)
        ...
        pdf.ln(3); pdf.set_font('Arial', 'B', 10); pdf.cell(0, 5, "CLAUSULA PRIMERA: OBJETO DEL CONTRATO", ln=True); pdf.set_font('Arial', '', 10)
        pdf.multi_cell(0, 5, f"EL PRESTADOR se compromete a realizar el servicio en el domicilio ubicado en {dom}. Detalle:"); pdf.ln(2)
    ...
    if f_c_img is not None:
        ...
        pdf.set_y(y_f + 22); pdf.set_font('Arial', 'B', 8); pdf.cell(95, 7, 'EL CLIENTE', 0, 0, 'C'); pdf.cell(95, 7, f'IVAN ORTIZ ({empresa_full})', 0, 1, 'C')
    return pdf.output(dest='S').encode('latin-1')

# 4. INTERFAZ OPERATIVA (NO SE MODIFICA LÓGICA EXISTENTE)
...
with st.container():
    ...
    if tipo == "Servicio IO Prevent":
        ...
        with st.expander("🛠️ CONFIGURACIÓN MANTENIMIENTO", expanded=True):
            ...
            opc_mnt = [f"Mantenimiento {c_m} Cam", f"Baluns {c_b} Par", f"Limpieza {sel_d}"]
            m_prorr = st.multiselect("💰 PRORRATEAR GANANCIA EN:", opc_mnt)
            act = len(m_prorr); ex = round(m_total / act, 2) if act > 0 else 0
            if c_m > 0: v = round((c_m*p_m)+(ex if f"Mantenimiento {c_m} Cam" in m_prorr else 0), 2); items_pdf.append({"Cantidad": c_m, "Concepto": f"Mantenimiento {sub_m} Camara", "Subtotal_Final": v}); total_final += v
            if c_b > 0: v = round((c_b*p_b)+(ex if f"Baluns {c_b} Par" in m_prorr else 0), 2); items_pdf.append({"Cantidad": c_b, "Concepto": "Reemplazo de Baluns", "Subtotal_Final": v}); total_final += v
            v_d = round(p_d_mnt+(ex if f"Limpieza {sel_d}" in m_prorr else 0), 2); items_pdf.append({"Cantidad": 1, "Concepto": f"Limpieza tecnica {sel_d}", "Subtotal_Final": v_d}); total_final += v_d
    elif m_sel:
...
if st.button("🚀 FINALIZAR Y GENERAR EXPEDIENTE"):
    if c_cli.image_data is not None and nom and dom_input:
        try:
            # CORRECCIÓN DE ARGUMENTOS PARA ELIMINAR ERROR EN ROJO
            ...
            msg = urllib.parse.quote(f"Hola {nom}, soy Ivan de IO SECURITY. Confirmo generacion de contrato {tipo}. Adjunto PDF.")
